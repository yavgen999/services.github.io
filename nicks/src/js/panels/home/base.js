...
import {Div, Panel, Alert, Group, Cell, Tabs, Search, List, PanelHeader, PanelHeaderButton, Separator, CardGrid, Card, Link, Placeholder, Input, FormLayoutGroup, FormLayout, Header, Button, Tooltip, PanelHeaderContent, ModalRoot, Snackbar, Avatar, Radio, Footer, Switch, Textarea, Select} from "@vkontakte/vkui";
    ...
    switch(value) {
      case 1: {
        ...
        this.setState({ snackbar:
            <Snackbar
              ...
              onActionClick={() => setPage('home','favorites')}
      ...
      case 3: {
        this.setState({ snackbar:
            <Snackbar
              onClose={() => this.setState({ snackbar: null })}
              before={<Avatar size={24}><Icon24Dismiss fill="#ff0000" width={14} height={14} /></Avatar>}
            >
              Такой nickname уже есть в Вашем избранном!
            </Snackbar>
          });
          return;
      }
...
async addToFavorites() {
  await bridge.send("VKWebAppStorageGet", {"keys": ["favorites"]}).then(data => {
    let checkFavorites = data.data.keys[0].value.split(";");
    for (var i = 0; i < checkFavorites.length; i++) {
      if(checkFavorites[i] == this.state.nick) return this.showSnackbar(3);
    }
    let favorites = data.data.keys[0].value + ';' + this.state.nick;
    bridge.send("VKWebAppStorageSet", {"key": "favorites", "value": favorites});
    this.showSnackbar(1);
  });
}
...
openPopout() {
    this.props.openPopout(
        ...
        >
            ...
            <Input type="text" value={this.state.value1} align="center" name="value1" onChange={this.onChanges} maxLength={3} top="Символы/слова, которые будут стоять между двумя словами (только для ника из двух слов). Максимум - 3 символа."/>
            </FormLayout>
        </Alert>
    );
}
...
copy = () => {
    navigator.clipboard.writeText(this.state.nick);
    this.showSnackbar(2);
}
    ...
    render() {
        ...
        return (
        <Panel id={id}>
                <PanelHeader left={<PanelHeaderButton style={{ cursor: 'pointer'}} onClick={() => setPage('home','favorites')}><Icon24FavoriteOutline/></PanelHeaderButton>}><PanelHeaderContent style={{ cursor: 'pointer' }} aside={<Icon24Filter/>} onClick={(this.pop)}>Генератор</PanelHeaderContent></PanelHeader>
                <Div>
        <div style={{
          ...
          borderRadius: 12
            ...
            <h1 style={{ fontFamily: ' monaco, sans-serif', color: '#FFF' }}>{this.state.nick}</h1>
                ...
                <Button style={{ marginRight: 'auto', cursor: 'pointer', marginLeft: 'auto', borderRadius: '53px', backgroundImage: 'linear-gradient(135deg, #ffb73d, #ffa000)'}} onClick={() => this.addToFavorites()}><Icon56FavoriteOutline fill='#FFF'/></Button>
                <Button style={{ marginRight: 'auto', cursor: 'pointer', borderRadius: '53px', backgroundImage: 'linear-gradient(135deg, #ffb73d, #ffa000)'}} onClick={(this.copy)}><Icon28CopyOutline fill='#FFF' height={51} width={51}/></Button>
                <Button style={{ marginRight: 'auto', cursor: 'pointer', borderRadius: '53px', backgroundImage: 'linear-gradient(135deg, #ffb73d, #ffa000)'}} onClick={(this.repost)}><Icon28ShareOutline fill='#FFF' height={51} width={51}/></Button>
                </Div>
                <Separator style={{ margin: '12px 3' }} />
                <Group header={<Header mode="secondary">Список генераторов</Header>}>
                <Div style={{display: 'flex'}}>
                <Button size="xl" mode="secondary" style={{ cursor: 'pointer', marginRight: 3, backgroundImage: 'linear-gradient( 135deg, #ABDCFF 10%, #0396FF 100%)', color: '#FFF', borderRadius: '53px' }} onClick={(this.generation)}>Английский</Button>
                <Tooltip
                ...
                cornerOffset={-6}
                >
                <Button size="l" mode="secondary" style={{ cursor: 'pointer', borderRadius: '50px', backgroundImage: 'linear-gradient( 150deg, #5EFCE8 10%, #736EFE 100%)', color: '#FFF' }} onClick={() => (this.setState({tooltip1: true}))} >?</Button>
                </Tooltip>
                </Div>
                <Div style={{display: 'flex'}}>
                <Button size="xl" mode="secondary" style={{ cursor: 'pointer', marginRight: 3, backgroundImage: 'linear-gradient( 135deg, #ABDCFF 10%, #0396FF 100%)', color: '#FFF', borderRadius: '53px' }} onClick={(this.generation2)}>Русский</Button>
                <Tooltip
                ...
                cornerOffset={-6}
                >
                <Button size="l" mode="secondary" style={{ cursor: 'pointer', borderRadius: '50px', backgroundImage: 'linear-gradient( 150deg, #5EFCE8 10%, #736EFE 100%)', color: '#FFF' }} onClick={() => (this.setState({tooltip2: true}))}>?</Button>
                </Tooltip>
                </Div>
                </Group>
                {this.state.snackbar}
